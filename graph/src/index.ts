...
class ClusterGraphContainer {
    ...
    // Graph elements
    private svg = this.container
        .append("svg");

    private graph = this.svg
        .append("g");

    // Input elements
    private flatValueElement = <HTMLInputElement>document.querySelector("#flatValue");
    private compactElement = <HTMLInputElement>document.querySelector("#compactCheck");

    // Cluster visualiser
    private clusterElement = <HTMLDivElement>document.querySelector("#clusters");
    private modelElement = <HTMLSelectElement>document.querySelector("#models-dropdown");
    ...
    public async run() {
        ...
        new Dendrogram(this.root, this.graph, this.flatValueElement, this.modelElement, this.models, containerSize);
    }
    ...
    private async getData() {
        const popup = d3.select("#fetchPopup")
            .style("display", "block");
    ...
    private maxClusterDistance() {
        d3.select("#flatSlider")
            .attr("min", d3.min(this.root.descendants(), d => d.data.distance) || 0)
            .attr("max", d3.max(this.root.descendants(), d => d.data.distance) || 0);
    }
